...
public sealed class ApiKeyServiceTests : IAsyncDisposable
{
    ...
    public async Task RevokeApiKeyAsync_WhenAlreadyRevoked_ShouldFail()
    {
        ...
        // Assert
        result.IsFailed.Should().BeTrue();
        result.Errors.Should().ContainSingle(e => e.Message.Contains("already revoked"));
    }
    ...
    [Fact]
    public async Task RegenerateApiKeyAsync_ShouldCreateNewKeyAndRevokeOld()
    {
        // Arrange
        var tenant = await CreateTestTenantAsync();
        var original = await _sut.CreateApiKeyAsync(tenant.Id, "Test Key", ApiKeyScope.WriteDocuments);
        var originalId = original.Value.KeyEntity.Id;

        // Act
        var result = await _sut.RegenerateApiKeyAsync(tenant.Id, originalId);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.PlainKey.Should().NotBe(original.Value.PlainKey);
        result.Value.KeyEntity.Name.Should().Be("Test Key");
        result.Value.KeyEntity.Scope.Should().Be(ApiKeyScope.WriteDocuments);

        // Old key should be revoked
        var oldKey = await _dbContext.ApiKeys.FindAsync(originalId);
        oldKey!.IsRevoked.Should().BeTrue();
        oldKey.RevokedReason.Should().Be("Regenerated");
    }

    [Fact]
    public async Task RegenerateApiKeyAsync_ShouldPreserveRateLimits()
    {
        // Arrange
        var tenant = await CreateTestTenantAsync();
        var original = await _sut.CreateApiKeyAsync(tenant.Id, "Test Key");

        // Modify rate limits
        var key = await _dbContext.ApiKeys.FindAsync(original.Value.KeyEntity.Id);
        key!.RequestsPerMinute = 100;
        key.RequestsPerDay = 5000;
        await _dbContext.SaveChangesAsync();

        // Act
        var result = await _sut.RegenerateApiKeyAsync(tenant.Id, original.Value.KeyEntity.Id);

        // Assert
        result.Value.KeyEntity.RequestsPerMinute.Should().Be(100);
        result.Value.KeyEntity.RequestsPerDay.Should().Be(5000);
    }

    [Fact]
    public async Task RegenerateApiKeyAsync_WhenKeyNotFound_ShouldFail()
    {
        // Arrange
        var tenant = await CreateTestTenantAsync();

        // Act
        var result = await _sut.RegenerateApiKeyAsync(tenant.Id, Guid.NewGuid());
